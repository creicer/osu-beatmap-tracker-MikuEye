...
from core import (
    _config_file, _default_sound,
    DEFAULT_CHECK_INTERVAL, _save_config_dir,
    COLOR_BG, COLOR_BG_LIGHT, COLOR_CARD, COLOR_ACCENT, COLOR_ACCENT_DARK,
    COLOR_TEXT, COLOR_TEXT_DIM, COLOR_SUCCESS, COLOR_WARNING, COLOR_DANGER,
    V2_STATUS_MAP, MODE_INFO, BEATMAP_STATUS,
    _BROWSE_STYLE_BASE, _BROWSE_STYLE_HOVER, _BROWSE_STYLE_SELECTED, _BROWSE_STYLE_DONE,
    _cover_pool, _cover_cache,
    MonitorWorkerThread, RefreshAllWorkerThread, BrowseQualifiedWorkerThread,
    load_config, save_config, get_oauth_token, get_beatmap_info, get_beatmap_cover_bytes,
...
class FirstRunDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self._dir_input.setPlaceholderText("Pick a folder or type a path...")
        ...
        self._dir_input.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self._dir_input.customContextMenuRequested.connect(
            lambda pos: _lineedit_context_menu(self._dir_input, pos, _menu_style_base()))
        ...
        self._dir_input.textChanged.connect(self._on_path_typed)

    def _on_path_typed(self, text):
        path = text.strip()
        if path and os.path.isdir(path):
            self.chosen_dir = path
    ...
    def closeEvent(self, event):
        # allow closing freely — main() checks chosen dir after exec()
...
class SettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self._cfg_dir_input = QLineEdit(_core._config_dir)
        ...
        self.auto_stop_checkbox = QCheckBox("Auto stop tracking when beatmap status changes")
        ...
        # if auto utc, compute local offset else use saved value
    ...
    def _browse_config_dir(self):
        folder = QFileDialog.getExistingDirectory(
            self, "choose config folder", _core._config_dir or os.path.expanduser('~')
        )
        if folder and folder != _core._config_dir:
    ...
    def save_settings(self):
        ...
        if new_dir and new_dir != _core._config_dir and os.path.isdir(new_dir):
            old_path = _config_file()
            _core._config_dir = new_dir
...
class HistoryDialog(QDialog):
    def __init__(self, history, client_id='', client_secret='', utc_offset=0, on_history_changed=None, parent=None):
        super().__init__(parent)
    ...
    def _delete_rows_by_filtered_indices(self, row_indices):
        # remove entries that correspond to given filtered history indices
    ...
    def import_history(self):
        ...
        try:
            ...
            # merge, avoid duplicates by timestamp+beatmap id key
    ...
    def fetch_missing_dates(self):
        ...
        class _FetchWorker(QThread):
            ...
            finished = pyqtSignal(dict)  # bid -> approved date or none
...
class MainWindow(QMainWindow):
    ...
    def create_beatmap_list(self):
        ...
        # keep _bulk row widget as invisible placeholder for layout compatibility
    ...
    def _on_unified_search(self):
        ...
        # map sort index to osu! api sort param
        ...
        _BROWSE_SORT_MAP = {
            ...
            2: 'ranked_desc',  # status (no direct api equiv, use ranked desc)
    ...
    def _browse_fetch_by_id(self, beatmap_id):
        # fetch a single beatmapset by id using get beatmap info, then show it
        ...
        def on_done(info):
            ...
            self._browse_loading = False
            # convert get beatmap info result into browse card format
    ...
    def _browse_fetch_page(self, cursor=None, status_override=None):
        # fire one api page request. status override used for parallel multi status fetches
    ...
    def update_beatmap_list(self, full_rebuild=False):
        try:
            ...
            # build sorted+filtered list of (original index, beatmap)
            ...
            STATUS_ORD = {'1': 0, '3': 1, '4': 2, '0': 3, '-1': 4, '-2': 5}
            if sort_option == 0:
    ...
    def show_history(self):
        dialog = HistoryDialog(
            ...
            on_history_changed=lambda: save_config(self.config),
            parent=self
        )
        dialog.exec()
